...
import com.smsapi.service.SessionEventHandler;
...
public class SessionRedisQWorker extends Thread {
	...
	public void run() {
		...
		if(sessionmanager.getDnWorkerMap().get(systemId)!=null) {
			sessionmanager.getDnWorkerMap().get(systemId).remove(this);
			if(sessionmanager.getDnWorkerMap().get(systemId).size()==0) {
				sessionmanager.getDnWorkerMap().remove(systemId);
			}
		}
	}
